...
document.getElementById("addamount").addEventListener("click", async () => {
    ...
    if (date.length == 0) {
        ...
        function pay(id) {
            var options = {
                "key": "rzp_live_3mWK5obhokVYg0",  //Enter your razorpay key ()  rzp_live_3mWK5obhokVYg0
                "currency": "INR",
                "name": "Smart Way",
        ...
        async function sendpay(res) {
            console.log(res)
            var paymentsend = await axios.post(`https://api.smartwayonline.in/payment/distributorpaymentssend`, {
                userid: userid,
            ...
            if (paymentsend !== true) return toastr["error"]("Something Wrong Contact SmartWay")
